...
企业级实例规格变配限制请参见[实例规格变配介绍](/cn.zh-CN/实例/升降配实例/实例规格变配介绍.md)。

## 共享型实例能否变配为企业级实例？

详情请参见[实例规格变配介绍](/cn.zh-CN/实例/升降配实例/实例规格变配介绍.md)。
...
详细的实例规格变配支持情况，请参见[实例规格变配介绍](/cn.zh-CN/实例/升降配实例/实例规格变配介绍.md)。

## 按量付费GPU实例支持停机不收费功能吗？

带本地存储的实例规格族不支持按量付费实例停机不收费功能，例如gn5。更多停机不收费说明请参见[按量付费实例停机不收费](/cn.zh-CN/产品定价/按量付费实例停机不收费.md)。
...
-   方式一
    1.  将固定带宽转换为按量带宽，具体操作请参见[公网带宽计费方式](/cn.zh-CN/产品定价/公网带宽计费方式.md)。
...
搭建支持PHP的Web环境，通常需要安装Apache/Nginx+PHP+MySQL。您可以通过[云市场](https://market.aliyun.com/)选购相关镜像，也可以手动搭建环境。详情请参见[部署LAMP环境](/cn.zh-CN/建站教程/搭建环境/部署LAMP环境.md)或[手动部署LNMP环境（Aliyun Linux）](/cn.zh-CN/建站教程/搭建环境/部署LNMP环境/手动部署LNMP环境（Aliyun Linux）.md)。
...
支持的地域请参见[地域和可用区](/cn.zh-CN/产品简介/地域和可用区.md)。
...
**欠费停机**指实例因账号欠费而自动停止服务，处于欠费停机状态的实例不计费。但账号欠费后，实例不会一直处于欠费停机状态，详情请参见[按量付费](/cn.zh-CN/产品定价/按量付费.md)。

**停机**指在实例正常运行期间，您通过ECS管理控制台或StopInstance接口，使实例进入**已停止**状态。此时，实例的计费情况和网络类型有关。

-   专有网络：和实例是否开启停机不收费有关。
    -   开启停机不收费：从创建开始计费，实例处于**已停止**状态时停止计费，实例启动后又重新计费。按量付费实例停机不收费功能只适用于实例的计算资源（即vCPU和内存）和固定公网IP地址，但云盘、EIP等其它资源仍然正常计费，详情请参见[按量付费实例停机不收费](/cn.zh-CN/产品定价/按量付费实例停机不收费.md)。
    -   未开启停机不收费：停机后仍然正常计费。
...
不变。按量付费转包年包月功能只支持转换实例和云盘的计费方式，带宽计费方式的变更方法请参见[升降配方式汇总](/cn.zh-CN/实例/升降配实例/升降配方式汇总.md)。
...
-   归属于您的账号下。
-   不能是已停售的实例规格，详情请参见[已停售的实例规格](/cn.zh-CN/实例/已停售的实例规格.md)。
-   不能是抢占式实例。
-   不能有未支付的转换订单。
